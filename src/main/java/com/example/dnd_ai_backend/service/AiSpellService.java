...
@Service
public class AiSpellService {

        private final SpellService spellService;
        private final ChatGptService chatGptService;

        public AiSpellService(SpellService spellService, ChatGptService chatGptService) {
                this.spellService = spellService;
                this.chatGptService = chatGptService;
        }

        public String getSpellExplanation(String index) {
        ...
        public String chatAboutSpell(String index, List<ChatGptRequest.Message> messages) {
                SpellDetailResponse spell = spellService.getSpellByIndex(index);
                String spellText = SpellFormatter.format(spell);
                String systemPrompt = """
                                You are an expert Dungeons & Dragons 5e rules explainer.
                                You are currently discussing the spell "%s".
                                Keep answers concise, accurate, and helpful.
                                If the user asks about something not related to the spell, politely steer the conversation back to the spell.
                                """
                                .formatted(spell.name());

                // Add prompt and spell data as system context
                ChatGptRequest.Message systemMessage = new ChatGptRequest.Message("system",
                                systemPrompt + "\n" + "Spell data:\n" + spellText);

                List<ChatGptRequest.Message> allMessages = new ArrayList<>();
                allMessages.add(systemMessage);
                allMessages.addAll(messages);

                ChatGptResponse response = chatGptService.sendRequest(allMessages);
